...
def generate_new_dataset(dataset, column_target:str, n_outputs:int):
    """
    ...
    d = sd.DataHub()
    d.load(dataset.copy())
    generated = d.generate(n_outputs, target=column_target, model=gen.KDE())

    return generated

def create_new_generated_dataset(dataset_name:str, generated) -> None:
    """
    ...
    """
    gen_path = get_generation_dir_path()
    new_name = dataset_name.split(".")[0]+"_generated.csv"
    new_new_name = new_name
    ind=1
    while(dataset_exist(gen_path, new_new_name)):
        ind += 1
        new_new_name = new_name.split(".")[0]+"_"+str(ind)+".csv"
    final_path = gen_path + "/" + new_new_name
    generated.to_csv(final_path, index=False)
    print("Dataset generated at:",final_path)

def fill_imported_dataset(dataset):
    """
...
def create_new_dataset_on_imported(dataset_name:str, filled_table) -> None:
    """
    Creates a new imported dataset after filling 
    ...
    """
    imported_path = get_import_dir_path()
    new_name = dataset_name.split(".")[0]+"_fill.csv"
    new_new_name = new_name
    ind=1
    while(dataset_exist(imported_path, new_new_name)):
        ind += 1
        new_new_name = new_name.split(".")[0]+"_"+str(ind)+".csv"
    final_path = imported_path + "/" + new_new_name
    filled_table.to_csv(final_path, index=False)
