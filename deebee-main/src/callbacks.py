...
from src.dataset_operations import (
    ...
    fill_imported_dataset,
    create_new_dataset_on_imported,
    generate_new_dataset,
    create_new_generated_dataset
)
...
def set_callbacks(app) -> dash.Dash:
    ...
    @app.callback(
        [
            ...
            Output("imported_datasets_tofill_dropdown", "options"),
            Output("imported_datasets_generate_dropdown", "options"),
    ...
    @app.callback(
        [
            Output("generate_dataset_modal", "is_open"),
            Output("imported_datasets_generate_dropdown", "value"),
        ],
        [
            Input("generate_button", "n_clicks"),
        ],
        [
            State("imported_datasets_generate_dropdown", "value"),
        ],
        prevent_initial_call=True
    )
    def toggle_generate_table_choser(
        open_modal: int, dataset_name
    ) -> (bool, str):
        """
        Opens the generation table screen when the button is clicked.

        :param open_modal: Number of clicks.
        :param dataset_name: String with the name of the dataset the user is generating more data for
        ...
        if is_trigger("generate_button"):
            is_open, dataset_name = True, EMPTY_STRING
        return is_open, dataset_name
    
    @app.callback(
        [
            Output("fill_dataset_modal", "is_open"),
            Output("imported_datasets_tofill_dropdown", "value"),
        ],
        [
            Input("fill_button", "n_clicks"),
        ],
        [
            State("imported_datasets_tofill_dropdown", "value"),
        ],
        prevent_initial_call=True
    )
    def toggle_fill_table_choser(
        open_modal: int, dataset_name
    ) -> (bool, str):
        """
        Opens the fill table screen when the button is clicked.

        :param open_modal: Number of clicks.
        :param dataset_name: String with the name of the dataset the user is filling
    ...
    @app.callback(
        Output("choose_column_target_to_generate_modal", "is_open", allow_duplicate=True),
        [
            Input("generate_selected_dataset_button", "n_clicks"),
        ],
        [
            State("imported_datasets_generate_dropdown", "value"),
            State("table_select_target_generate_dropdown", "value"),
            State("generation_set_n", "value"),
            State("choose_column_target_to_generate_modal", "is_open")
        ],
        prevent_initial_call=True
    )
    def generate_dataset(
        ...
        """
        Fill NAs of the current dataset

        :param new_target: Number of clicks.
        :param dataset_name: Currently selected dataset or table.
        :param column_target_name: Currently selected dataset column.
        :param n_outputs: Number of outputs for each class in the new dataset.
        :param modal_state: Current modal state.

        :return: Bool.
        """
        if is_trigger("generate_selected_dataset_button"):
            if dataset_name:
                dataset_path = get_imported_dataset_path(dataset_name)
                sep = infer_csv_separator(dataset_path)
                table = read_dataset(dataset_path, sep=sep)
                generated_table = generate_new_dataset(table, column_target_name, int(n_outputs))
                if len(generated_table) > 1:
                    modal_state = False
                    create_new_generated_dataset(dataset_name=dataset_name, generated=generated_table)
        else:
            modal_state = True
        return modal_state
    ...
    @app.callback(
        [    
            Output("fill_dataset_modal", "is_open", allow_duplicate=True),
            Output("imported_datasets_checklist", "options", allow_duplicate=True),
        ],
        [
            Input("set_target_button", "n_clicks"),
        ],
        [
            State("imported_datasets_tofill_dropdown", "value"),
            State("fill_dataset_modal", "is_open")
        ],
        prevent_initial_call=True
    )
    def fill_dataset(
        ...
        if is_trigger("set_target_button"):
            if dataset_name:
                dataset_path = get_imported_dataset_path(dataset_name)
                sep = infer_csv_separator(dataset_path)
                table = read_dataset(dataset_path, sep=sep)
                filled_table = fill_imported_dataset(table)
                if len(filled_table) == len(table) and len(filled_table.columns) == len(table.columns):
                    modal_state = False
                    create_new_dataset_on_imported(dataset_name=dataset_name, filled_table=filled_table)
                    available_options=refresh_imported_dataset_listing()
        else:
            modal_state = True
        return( 
    ...
    @app.callback(
        Output("choose_column_target_to_generate_modal", "is_open"),
        [
            Input("set_target_generation_button", "n_clicks"),
        ],
        [
            State("imported_datasets_generate_dropdown", "value"),
            State("choose_column_target_to_generate_modal", "is_open")
    ...
    def toggle_select_target_generation_data(
        new_target: int,
        dataset_name: str,
        modal_state
    ) -> bool:
        """
        Opens the selection target definer to generate when the button is clicked, only if conditions
        match.
        ...
        :return: Bool.
        """
        if is_trigger("set_target_generation_button"):
            if dataset_name:
                modal_state = True
        else:
            modal_state = False
    ...
    @app.callback(
        Output("table_select_target_generate_dropdown", "options")
        ,
        [
            Input("imported_datasets_generate_dropdown", "value")
        ]
    )
    def find_available_table_columns_to_generate(
        dataset_name: str
    ) -> (list):
        """
        Finds table columns once the user selects one dataset to generate new data on.
